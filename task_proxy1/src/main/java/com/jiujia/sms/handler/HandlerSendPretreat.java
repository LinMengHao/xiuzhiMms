...
import static com.jiujia.conf.GlobalBlackFilterConfig.*;
			...
			while((jsonStr = RedisUtils.fifo_pop(fifo_key))!=null){
				...
				for (int i=0;i<phonesArr.length;i++){
					...
					if (BLACK_SORT.size()==0){
						mobJson = ThreadConstants.mobileCheck(mobile,companyId,appId);
					}else {
						for (int j = 0; j < BLACK_SORT.size(); j++) {
							//命中一个就锁死
							if(mobJson.getInteger("check")==2){
								break;
							}
							if("JS".equals(BLACK_SORT.get(j))){
								...
								mobJson = ThreadConstants.mobileCheckNj(mobile,companyId,appId,NJ_USERNAME,NJ_PASSWORD,"1",NJ_SERVICEURL);
							}else if("DYV".equals(BLACK_SORT.get(j))){
								logger.info("黑名单过滤选择：{}","DYV");
								mobJson=ThreadConstants.mobileCheckDy(mobile,companyId,appId,DY_VOICE_ACCESSKEY,DY_VOICE_SECRETKEY,DY_VOICE_SERVICEURL);
							}else if("DYD".equals(BLACK_SORT.get(j))){
								logger.info("黑名单过滤选择：{}","DYD");
								mobJson=ThreadConstants.mobileCheckDy(mobile,companyId,appId,DY_DX_ACCESSKEY,DY_DX_SECRETKEY,DY_DX_SERVICEURL);
							}
						}
					}

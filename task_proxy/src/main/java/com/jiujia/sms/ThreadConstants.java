...
public class ThreadConstants {
	...
	public static JSONObject mobileCheck(String mobile,String companyId,String appId){
		...
		//黑名单校验
		if(mobileBlack(mobile,companyId,appId)){
			json.put("check",2);//1-白名单 2-受限 3-未受限
			//黑名单
			json.put("status","BLACK");//状态
			json.put("statusName","黑名单");//状态描述
	...
	//黑名单校验
	private static boolean mobileBlack(String mobile,String companyId,String appId){
		//默认过滤江苏...
		if (BLACK_SORT.size()==0){
			...
			if(GlobalBlackFilterConfig.JS_BLACK_LEVEL.size()==0){
				//默认使用最高级别
				if(result==30||result==31||result==32||result==33){
					//黑名单
					return true;
				}
			}else {
...
//		paramMap.put("sign",sign);
	...
	private static int mobileCheckDy(String mobile,String companyId,String appId,String accessKey,String secretKey,String url){
		...
		String str = HttpInterface.httpClientPost(url, paramMap.toJSONString(), 30000, "UTF-8",formHeaders);
		//重试
		if(str.indexOf("http请求异常")==0){
			str = HttpInterface.httpClientPost(url, paramMap.toJSONString(), 30000, "UTF-8",formHeaders);
		}
		int blackStatus= -1;
		if(str.indexOf("http请求异常")==0){
			logger.info("黑名单三方校验提交结果异常：{}：{}",mobile,str);
